...
class Metrics():
    ...
    # Funding Ratio:
    def funding_ratio(self, assets: pd.Series, liabilities: pd.Series, r: float) -> float:
        """
    ...
    def bond_cash_flows(self, maturity: float, principal: float=100, coupon_rate: float =0.03, coupons_per_year: int =12) -> pd.Series:
        """
        Returns a series of cash flows generated by a bond, indexed by a coupon number
        principal is alco called the face value of the bond
        """
    ...
    def bond_price(self, maturity: float, principal: float=100, coupon_rate: float =0.03, coupons_per_year: int =12, discount_rate: float=0.03) -> float:
        """
        Price a bond based on bond params: maturity, principal, coupon rate, coupons_per_year and the prevailing discount_rate
        discount_rate is the current interest rate
        """
    ...
    def match_duration(self, cf_t:pd.Series, cf_s:pd.Series, cf_l:pd.Series, discount_rate: float) -> float:
        """
